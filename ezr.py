...
def twoFar(self: DATA, rows: rows, sortp=False, samples: int = None) -> tuple[
    ...
    if sortp and self.chebyshev(right) < self.chebyshev(
...
def activeLearning(self: DATA, score=lambda B, R: B - R, generate=None, faster=True,
                   label=None):
    def ranked(rows):
        return self.clone(rows).chebyshevs().rows
    ...
    if label:
        todo, done = self.rows[label:], ranked(self.rows[:label])
    else:
...
class egs:
    ...
    def diversities(d=None):
        ...
        print(d.chebyshev(
            d.clone([row for row in d.diversity(stop=10)]).chebyshevs().rows[0]))
                ...
                for some in [64, 128, 256, 512, 10000000000]:
                    for train, test in xval(d.rows, m=5, n=5, some=some):
                        ...
                        for want in test:
                            ...
                            for at, got1 in got.items():
                                sd = d.cols.all[at].div()
                                print(k, some, stop, (want[at] - mid[at]) / sd,
                                      (want[at] - got1) / sd, sep=",")
    ...
    def _mqs():
        ...
        for n in [15, 20, 25, 30, 50, 100]:
            ...
            start = time()
            mqs1000 = [
                rnd(d.chebyshev(d.shuffle().activeLearning(generate=generate1)[0]))
                for _ in range(repeats)]
    ...
    def policies():
        ...
        scoring_policies = [('exploit', lambda B, R,: exploit(B, R)),
                            ...
                            ('SimAnneal', lambda B, R: ((
                                                                    exp(B) + 1) ** normalized_exp(
        ...
        for what, how in scoring_policies:
            for the.divide in [0, 1]:
                ...
                for the.Last in [20, 30, 40]:
                    start = time()
                    result = [
                        rnd(d.chebyshev(d.shuffle().activeLearning(score=how)[0]))
                        for _ in range(repeats)]
                    print(
                        f"{what}/branch={divide}.{the.Last}: {(time() - start) / repeats:.2f} secs")
                    somes += [
